...
@interface GetSeedJSONResponseSerializerTests : XCTestCase {
    Method _method;
    IMP _originalIMP;
}

@property (nonatomic,strong) GetSeedJSONResponseSerializer* responseSerializer;
@end

@implementation GetSeedJSONResponseSerializerTests

- (void)setUp
{
    [super setUp];
    _method = class_getInstanceMethod([AFJSONResponseSerializer class], @selector(responseObjectForResponse:data:error:));
    _originalIMP = method_setImplementation(_method, (IMP)response);
    self.responseSerializer = [[GetSeedJSONResponseSerializer alloc] init];
}
...
- (void)test_ShouldReturnNil_WhenNSDictionaryWithoutCapcthaFieldReturnedFromAFNetworking
{
    dataResponse = @{ @"seed" : @"seed"};
...
- (void)test_ShouldReturnSeedWithCaptcha_WhenNSDictionaryReturnedFromAFNetworking
{
